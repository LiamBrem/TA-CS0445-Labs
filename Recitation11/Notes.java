...
public class PrimQ1WithIterator<T> implements QueueWithIteratorInterface<T> {
	...
	public PrimQ1WithIterator(int initsize) {
	...
	public Iterator<T> iterator() {
		return new PrimQIterator();

	}

	private class PrimQIterator implements Iterator<T> {
		private int current;

		public PrimQIterator() {
			// start at front - set current
		}

		public boolean hasNext() {
			// if there's still more elements to go in the queue, return True
		}

		public T next() {
			// if we can't go to the next element (i.e. there are none left)
			// throw NoSuchElementException
			// otherwise return the current element and "move" our pointer up
		}
	}
